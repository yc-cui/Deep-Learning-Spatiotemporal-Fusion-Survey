#LibSTFv1/model/EDCSTFN/EDCSTFN.py
...
class EDCSTFNModel(BaseModel):
    def __init__(self,
        ...
        # 延迟初始化CompoundLoss，避免在构造时就绑定pretrained
    ...
    def get_compound_loss(self):
        """懒加载CompoundLoss，确保pretrained模型已经准备好"""
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch < self.pre_epoch:
            ...
            # 使用懒加载的CompoundLoss
            loss = self.get_compound_loss()(pred, gt)  
            opt.zero_grad()
    ...
    def on_train_epoch_end(self):
        ...
        # 在预训练阶段结束时，确保CompoundLoss已经初始化
        # 移除了重新赋值pretrained的代码，避免状态不一致
        if self.current_epoch == self.pre_epoch - 1:
            # 确保CompoundLoss在转换阶段就被初始化
            _ = self.get_compound_loss()
            print(f"预训练阶段结束，CompoundLoss已初始化。下一个epoch将开始主模型训练。")